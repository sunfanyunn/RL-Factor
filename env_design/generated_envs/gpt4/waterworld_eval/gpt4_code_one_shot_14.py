...
class Agent(pygame.sprite.Sprite):
    ...
    def move(self, direction):
        ...
        if direction[pygame.K_RIGHT]:
            x += GRID_SIZE
        self.rect.topleft = max(0, min(x, WIDTH - self.rect.width)), max(0, min(y, HEIGHT - self.rect.height))
...
class Circle(pygame.sprite.Sprite):
    def __init__(self, color):
        ...
        pygame.draw.circle(
            self.image, color, (CIRCLE_RADIUS, CIRCLE_RADIUS), CIRCLE_RADIUS
        )
        self.rect = self.image.get_rect()
    ...
    def reset(self):
        ...
        self.color = GREEN if random.choice([True, False]) else RED
