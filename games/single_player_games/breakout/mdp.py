...
def move_paddle_logic(state_manager, action):
    """Move the paddle based on the action (left or right)"""
    if action == "left" and state_manager.paddle["x"] > 0:
        state_manager.paddle["x"] -= 15
    elif (
        action == "right"
...
def main():
    ...
    while running:
        ...
        if keys[pygame.K_LEFT]:
            move_paddle_logic(state_manager, "left")
        if keys[pygame.K_RIGHT]:
            move_paddle_logic(state_manager, "right")

        ball_logic(state_manager)
